...
OVERALL_DIR = BASE_DIR / "outputs" / "renders" / "incoming_grad_grouped_college_latest"
OVERALL_SVG = OVERALL_DIR / "incoming_grad_grouped_college_latest.svg"
TAPERED_OVERALL_SVG = OVERALL_DIR / "incoming_grad_grouped_college_latest_tapered.svg"
OVERALL_PNG = OVERALL_DIR / "incoming_grad_grouped_college_latest.png"
KARYOTYPE = OVERALL_DIR / "karyotype.txt"
LINKS = OVERALL_DIR / "links.txt"
SITE_ROOT = BASE_DIR / "site" / "incoming_grad_grouped_college_dashboard_images"
...
SITE_INCOMING_COLLEGES = SITE_ROOT / "incoming_colleges"
SITE_GRADUATING_COLLEGES = SITE_ROOT / "graduating_colleges"
...
def selected_link_indices_for_nodes(kind: str, node_ids: set[str], link_pairs: list[tuple[str, str]]) -> set[int]:
    if kind == "incoming":
        return {index for index, (source_id, _target_id) in enumerate(link_pairs) if source_id in node_ids}
    return {index for index, (_source_id, target_id) in enumerate(link_pairs) if target_id in node_ids}
...
def ideogram_fill_paths(root: ET.Element, nodes: list[Node]) -> dict[str, ET.Element]:
    ideograms = root.find(".//svg:g[@id='ideograms']", NS)
    if ideograms is None:
        raise ValueError("Could not find ideograms group in grouped incoming_grad SVG.")

    filled_paths = []
    for path in ideograms.findall("svg:path", NS):
        style_dict = style_to_dict(path.get("style"))
        fill = style_dict.get("fill", "")
        if fill and fill.lower() != "none":
            filled_paths.append(path)

    if len(filled_paths) != len(nodes):
        raise ValueError(
            f"Ideogram/node mismatch: found {len(filled_paths)} filled ideogram paths for {len(nodes)} nodes."
        )

    return {node.node_id: path for node, path in zip(nodes, filled_paths)}


def incoming_group_fill_map(root: ET.Element, nodes: list[Node]) -> dict[str, str]:
    fill_paths = ideogram_fill_paths(root, nodes)
    group_fills: dict[str, str] = {}
    for node in nodes:
        if node.kind != "incoming":
            continue
        suffix = safe_name_from_node_id(node.node_id)
        style_dict = style_to_dict(fill_paths[node.node_id].get("style"))
        fill = style_dict.get("fill")
        if fill:
            group_fills[suffix] = fill
    return group_fills


def recolor_selected_college_ideograms(
    root: ET.Element,
    nodes: list[Node],
    college_key: str,
    group_fill_map: dict[str, str],
) -> None:
    fill_paths = ideogram_fill_paths(root, nodes)
    for node in nodes:
        suffix = safe_name_from_node_id(node.node_id)
        if COLLEGE_KEY_BY_NODE_SUFFIX.get(suffix) != college_key:
            continue
        fill = group_fill_map.get(suffix)
        if not fill:
            continue
        style_dict = style_to_dict(fill_paths[node.node_id].get("style"))
        style_dict["fill"] = fill
        fill_paths[node.node_id].set("style", dict_to_style(style_dict))
...
def output_path_for_college(kind: str, college_key: str) -> Path:
    filename = f"{college_key}.svg"
    if kind == "incoming":
        return SITE_INCOMING_COLLEGES / filename
    return SITE_GRADUATING_COLLEGES / filename
...
def write_college_focus_svg(
    ...
    node_ids = {
        ...
        for node in nodes
        if node.kind == kind
        and COLLEGE_KEY_BY_NODE_SUFFIX.get(safe_name_from_node_id(node.node_id)) == college_key
    }
    if not node_ids:
        raise ValueError(f"No {kind} grouped majors found for college '{college_key}'")
    keep_indices = selected_link_indices_for_nodes(kind, node_ids, link_pairs)
    if not keep_indices:
        raise ValueError(f"No ribbons found for {kind} college '{college_key}'")
    output_root = copy.deepcopy(base_root)
    hide_unselected_ribbons(output_root, keep_indices)
    recolor_selected_college_ideograms(output_root, nodes, college_key, group_fill_map)
    output_path.parent.mkdir(parents=True, exist_ok=True)
    ET.ElementTree(output_root).write(output_path, encoding="utf-8", xml_declaration=True)


def college_keys_for_kind(nodes: list[Node], kind: str) -> list[str]:
    college_keys = {
        COLLEGE_KEY_BY_NODE_SUFFIX[safe_name_from_node_id(node.node_id)]
        for node in nodes
        if node.kind == kind and safe_name_from_node_id(node.node_id) in COLLEGE_KEY_BY_NODE_SUFFIX
    }
    return sorted(college_keys)
...
def main() -> None:
    ...
    nodes = load_nodes()
    group_fill_map = incoming_group_fill_map(tapered_root, nodes)
    ...
    if args.all:
        ...
        incoming_college_keys = college_keys_for_kind(nodes, "incoming")
        graduating_college_keys = college_keys_for_kind(nodes, "graduating")
        for college_key in incoming_college_keys:
            write_college_focus_svg(
                tapered_root,
                "incoming",
                college_key,
                nodes,
                link_pairs,
                group_fill_map,
                output_path_for_college("incoming", college_key),
            )
        for college_key in graduating_college_keys:
            write_college_focus_svg(
                tapered_root,
                "graduating",
                college_key,
                ...
                link_pairs,
                group_fill_map,
                output_path_for_college("graduating", college_key),
            )
        ...
        print(f"Generated incoming college previews: {len(incoming_college_keys)}")
        print(f"Generated graduating college previews: {len(graduating_college_keys)}")
