...
Output directory
----------------
  circos_refresh_v2/outputs/renders/incoming_grad_grouped_college_latest/
"""
...
OUTPUT_DIR = BASE_DIR / "outputs" / "renders" / "incoming_grad_grouped_college_latest"
...
def normalize_college_bin(value: object) -> str:
    college = normalize_code(value).upper()
    if college == "OCB":
        return "OCB"
    if college == "CAS":
        return "CAS"
    if college == "COH":
        return "COH"
    if college in {"SOE", "ENGR", "ENGINEERING"}:
        return "SOE"
    if college in {"EDUC", "CAS + EDUC", "SOE + EDUC"}:
        return "EDUC"
    return "OTHER"


def build_group_college_bins(major_df: pd.DataFrame) -> dict[str, str]:
    counts: dict[str, Counter[str]] = defaultdict(Counter)
    for _, row in major_df.iterrows():
        group = str(row.get("group", "")).strip()
        if not group:
            continue
        counts[group][normalize_college_bin(row.get("college", ""))] += 1

    group_to_college: dict[str, str] = {}
    for group in VISUAL_GROUP_ORDER:
        if group not in counts:
            continue
        dominant_college, _count = counts[group].most_common(1)[0]
        group_to_college[group] = dominant_college
    return group_to_college


def assign_group_colors(group_to_college: dict[str, str]) -> dict[str, tuple[int, int, int]]:
    grouped: dict[str, list[str]] = defaultdict(list)
    for group in VISUAL_GROUP_ORDER:
        college = group_to_college.get(group)
        if college:
            grouped[college].append(group)

    assigned: dict[str, tuple[int, int, int]] = {}
    for college, groups in grouped.items():
        palette = COLLEGE_COLOR_PALETTES[college]
...
def main() -> None:
    ...
    group_to_college = build_group_college_bins(major)
    ...
    inc_colors = assign_group_colors(group_to_college)
...
<image>
dir   = .
  file  = incoming_grad_grouped_college_latest.png
png   = yes
svg   = yes
radius         = 2000p
background     = white
...
"""
    ...
    for group in inc_order:
        print(
            f"  [{group_to_college.get(group, 'OTHER'):5}] "
            f"[{GROUP_TO_FAMILY[group][:28]:28}] {group}  size={inc_totals[group]}"
        )
